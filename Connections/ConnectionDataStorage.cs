...
{
    public static class ConnectionDataStorage
    {
        ...
        public static List<ConnectionData> LoadSavedSettings()
        {
            ...
            foreach (var serializedSettingsString in settingsString.Split(new[] {STORAGE_ITEM_SEPARATOR},
        ...
        {
            var settings = LoadSavedSettings().Where(s => s.SerializedString != settingToDelete.SerializedString)
                .ToList();
            WriteSettingsToFile(settings);
        ...
        {
            var settingsString =
                string.Join(STORAGE_ITEM_SEPARATOR, settings.Select(s => s.SerializedString).ToArray());
